...
export default class Siema {
  ...
  init() {
    ...
    this.sliderFrame.style.width = `${this.selectorWidth /
      this.perPage *
      this.innerElements.length}px`;
    this.sliderFrame.style.webkitTransition = `all ${this.config
      .duration}ms ${this.config.easing}`;
    this.sliderFrame.style.transition = `all ${this.config.duration}ms ${this
      .config.easing}`;
  ...
  insert(item, index, callback) {
    ...
    this.currentSlide =
      index <= this.currentSlide ? this.currentSlide + 1 : this.currentSlide;
